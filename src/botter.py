...
async def start_event_handler(event):
    ...
    await event.respond(
        f"hello **{user_name}** and welcome to pix2txt, please send a picture to extract the text."
    )
...
async def pic_event_handler(event):
    user_id, user_name = await get_id_user_name(event)
    logger.info(
        f"checking if user_id {user_id} user_name {user_name} did send a picture"
    )
    ...
    if not pic_file:
        logger.warning(
            f"user_id {user_id} user_name {user_name} didn't send a proper picture"
        )
        await event.reply(
            "Please send a picture file you wish to convert to text, nothing else"
        )
        await event.delete()
    else:
        logger.info(
            f"user_id {user_id} user_name {user_name} has sent a proper picture"
        )
        loading_gif = await event.reply(
            "downloading and analyzing picture, PLEASE WAIT", file="loading.gif"
        )
        ...
        logger.info(
            f"successfully sent extracted text to user_id {user_id} user_name {user_name}"
        )
...
async def get_id_user_name(event):
    ...
    logger.info("extracting user_id and user_name from event")
    if event.chat.username:
        logger.info(f"username exists {event.chat.username}")
        user_name = event.chat.username
    elif event.chat.first_name:
        logger.info(
            f"username DOESN'T exists, using first_name instead {event.chat.first_name}"
        )
        user_name = event.chat.first_name
    else:
        logger.info(
            "neither username nor first_name exist, using unknown username instead"
        )
...
if __name__ == "__main__":
    with botter:
        ...
        try:
            ...
            logger.exception(
                f"hit flood wait error -- {fwe}, gotta sleep for {fwe.seconds}",
                exc_info=True,
            )
            time.sleep(fwe.seconds)
            logger.info("attempting to re-start the bot")
            botter.run_until_disconnected()
        except errors.FloodError as fe:
            logger.exception(
                f"hit flood error -- {fe} with message -- {fe.message}", exc_info=True
            )
            time.sleep(5000)
            logger.info("attempting to re-start the bot")
            botter.run_until_disconnected()
            ...
            logger.warning("received EXITCMD, exiting")
            botter.disconnect()
            logger.info("bot disconnected, closing bot script")
            sys.exit(0)
        else:
            logger.info("successfully started the bot, starting operations")
